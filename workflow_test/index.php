...
if ($config['backward_support']) {
  ...
  if ($config['forum'] != '') {
    $config['forum_link'] =
      strtolower($config['forum']) === 'site' ? getLink('forum') : $config['forum'];
  }
...
if ($load_it) {
  if (SITE_CLOSED && admin()) {
    $content .=
      '<p class="note">Site is under maintenance (closed mode). Only privileged users can see it.</p>';
  }
...
$title_full =
  (isset($title) ? $title . $config['title_separator'] : '') . $config['lua']['serverName'];
require $template_path . '/' . $template_index;

echo base64_decode(
  'PCEtLSBQb3dlcmVkIGJ5IE9wZW5UaWJpYUJSIE15QUFDIDo6IGh0dHBzOi8vZ2l0aHViLmNvbS9vcGVudGliaWFici9teWFhYyAtLT4='
) . PHP_EOL;
if (superAdmin()) {
